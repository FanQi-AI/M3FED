...
CURRENT_DIR = Path(__file__).parent.abspath() #值Path('/home/tjut_lishuai/Per-FedAvg-master/data')
...
def preprocess(args: Namespace) -> None:
    ...
    num_train_clients = int(args.client_num_in_total * args.fraction) #0.9
    ...
    for client_id, dataset2 in enumerate(all_datasets):
        with open(pickles_dir / str(client_id) + ".pkl", "wb") as f:
            pickle.dump(dataset2, f)
        with open(pickles_dir / str(client_id) + ".pkl", "rb") as f:    
            loaded_obj = pickle.load(f)
            print(666)
        # with open(pickles_dir / str(client_id) + ".pt", "wb") as f:
        # torch.save(dataset,pickles_dir / str(client_id) + ".pt")
...
def randomly_alloc_classes(
    ...
    dict_users = noniid_slicing(ori_dataset, num_clients, num_clients * num_classes)  #len(dict_users) 180  存数据编号吧
    stats = {}
...
if __name__ == "__main__":
    with open("/data/mnist/pickles/0.pkl", "rb") as f:    
        loaded_obj = pickle.load(f)
        print(666)
